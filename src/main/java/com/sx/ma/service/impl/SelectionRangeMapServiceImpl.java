 ...
 */
@Service
public class SelectionRangeMapServiceImpl implements SelectionRangeMapService {
    ...
    public ResponseResult map(RequestVo requestVo) {
        ...
        // 选取范围
        String lowerResult = requestVo.getLowerResult();
        String upperResult = requestVo.getUpperResult();
        // 拼接成数据库表名
        ...
        if (!Objects.isNull(instrumentNameList)) {
            for (String instrumentName : instrumentNameList) {
                ...
                // 通过仪器名称查询数据集合
                List<InstrumentVo> instrumentVoList = scatterMapMapper.selectByInstrumentName(tableName, instrumentName);
                // 仪器最低数量大于等于50
                if (instrumentVoList.size() >= 50) {
                    classifiedInstrumentLocalCache.put(instrumentName, instrumentVoList);
                }
                if (!Objects.isNull(instrumentVoList) && upperResult != null && lowerResult != null) {
